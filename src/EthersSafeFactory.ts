...
class EthersSafeFactory {
    ...
    static async createSafe(safeSingletonAddress: string, signer: Signer, owners: string[], threshold?: number): Promise<Safe> {
        ...
        const proxyFactoryFactory = new ContractFactory(GnosisSafeProxyFactory.abi, GnosisSafeProxyFactory.bytecode, signer)
        ...
        await gnosisSafe.setup(owners, threshold, ZERO_ADDRESS, "0x", ZERO_ADDRESS, ZERO_ADDRESS, 0, ZERO_ADDRESS)
